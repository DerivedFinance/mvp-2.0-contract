...
contract MarketMaker is Ownable, ERC1155Receiver {
    ...
    event AMMResumed();
    event AMMClosed();
    event AMMFundingChanged(uint256 fundingChange);
    ...
    function calcNetCost(uint256[] memory outcomeTokenAmounts)
        public
        view
        virtual
    ...
    function changeFunding(uint256 fundingChange)
        public
        onlyOwner
    ...
    function trade(uint256[] memory outcomeTokenAmounts, uint256 collateralLimit)
        public
        ...
        returns (uint256 netCost)
    {
        ...
        uint256 outcomeTokenNetCost = calcNetCost(outcomeTokenAmounts);
        uint256 fees;
        if (outcomeTokenNetCost < 0)
            fees = uint256(calcMarketFee(uint256(-outcomeTokenNetCost)));
        else fees = uint256(calcMarketFee(uint256(outcomeTokenNetCost)));
