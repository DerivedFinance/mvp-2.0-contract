...
=======
  ...
  function getMarketVolume(uint256 _questionId) public view returns (uint256) {
    return markets[_questionId].volume;
  }

  function getTradeVolume(uint256 _questionId) public view returns (uint256) {
    return
      getMarketVolume(_questionId).sub(questions[_questionId].initialLiquidity);
  }
